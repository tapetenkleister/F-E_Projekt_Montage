...
def get_matrix(image, circles, matrix_Type):
    ...
    for row in grids: #cutted_grids  
        ...
        for point in row:        
            ...
            color_alt = get_avarege_color(point, im)      
            color_name_row.append(color)        
            color_name_row_alt.append(color_alt)
        color_name_grid.append(color_name_row)
        color_name_grid_alt.append(color_name_row_alt)
        index += 1

    #display_lego_pattern(color_name_grid)
   
    return color_name_grid,color_name_grid_alt, cutted_grids
...
def get_similarity(picture_grid, plan_grid):
    ...
    return similarity, (index_x+round(0.5*len(plan_grid))-1), (index_y+round(0.5*len(plan_grid))-1),comp_list
